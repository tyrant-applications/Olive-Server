...
def send_activation_mail(user,user_profile):
    ...
    context = Context()
    context['activation_link'] = settings.SERVICE_BASE_URL + '/confirm/?key='+activation_key
    email_body = t.render(context)
    email_subject = smart_unicode('Tyrant 계정 인증을 완료해 주세요!', encoding='utf-8', strings_only=False, errors='strict')
...
def send_invitation_mail(from_user, target_emails):
    ...
    context['link'] = settings.SERVICE_BASE_URL
    email_body = t.render(context)
    email_subject = smart_unicode('Tyrant를 사용해 보세요!', encoding='utf-8', strings_only=False, errors='strict')
...
def register_email_queue(target_email,subject,contents):
    try:
        if target_email != '' and target_email is not None:
            ...
            from_user = 'Tyrant<'+settings.EMAIL_HOST_USER+'>'
            msg = EmailMessage(subject, contents, from_user, [receiver])
            msg.content_subtype = "html"  # Main content is now text/html
            msg.send()
#            msg = EmailMultiAlternatives(subject, '', from_user, [receiver])
